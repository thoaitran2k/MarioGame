...
void LoadAssetsBrickQuestion()
{
	CTextures* textures = CTextures::GetInstance();
	CSprites* sprites = CSprites::GetInstance();
	CAnimations* animations = CAnimations::GetInstance();

	LPTEXTURE texMisc1 = textures->Get(ID_TEX_MISC);
	sprites->Add(20010, 300, 117, 317, 133, texMisc1);
	sprites->Add(20020, 318, 117, 335, 133, texMisc1);
	sprites->Add(20030, 336, 117, 353, 133, texMisc1);
	sprites->Add(20040, 354, 117, 371, 133, texMisc1);
	...
	ani->Add(20030);
	ani->Add(20040);
	animations->Add(ID_ANI_BRICK_Q, ani);
...
void Reload()
{
	...
	for (int i = 0; i < 5; i++)
	{
		CCoin* c = new CCoin( 150.0f +(COIN_X +i * (COIN_WIDTH * 2)), GROUND_Y - 96.0f);
		objects.push_back(c);
	}

	//BRICK_Q
	
		CBrickQuestion* q = new CBrickQuestion(150.0f, GROUND_Y - 92.0f);
		objects.push_back(q);
