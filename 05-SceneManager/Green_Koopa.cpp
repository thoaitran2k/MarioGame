...
CGreen_Koopa::CGreen_Koopa(float x, float y) :CGameObject(x, y)
{
	...
	SetState(KOOPA_GREEN_STATE_JUMP_RIGHT);
	isTurtleShell = false;
	startX = x;
	startY = y;
	this->rangeFly = startY - 20;
	isOnPlatform = false;
...
void CGreen_Koopa::OnCollisionWithPlatForm(LPCOLLISIONEVENT e)
{
	...
	if (e->ny < 0) {
		...
		isOnPlatform = true;
		if (state != KOOPA_GREEN_STATE_WALKING && state != KOOPA_GREEN_STATE_ISTURTLESHELL) {

			Jumping = true;
			//DebugOut(L">>> MMMMMMMMMMMMMMMMMMMM >>> \n");
	...
	if (!isDead) {
			...
			//if (isOnPlatform) {
				...
				if (!Jumping) {
					...
					if (state != KOOPA_GREEN_STATE_WALKING && !isTurtleShell) {
